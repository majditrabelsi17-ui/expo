...
function extractLabel(children) {
    const parts = react_1.Children.toArray(children);
    const label = parts.filter((child) => typeof child === 'string').join('');
    if (process.env.NODE_ENV !== 'production' && parts.length > 0 && !label) {
        console.warn('Stack.Toolbar.MenuAction on Android only supports string children. React elements will be ignored. Pass a plain string as children instead.');
    }
    return label;
}
/**
