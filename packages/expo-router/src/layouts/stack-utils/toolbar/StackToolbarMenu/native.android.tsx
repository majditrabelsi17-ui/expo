...
import { Children, createContext, use, useCallback, useState, type ReactNode } from 'react';
...
/**
 * Extracts string label from children (string children or nested text).
 */
function extractLabel(children: ReactNode): string {
  const parts = Children.toArray(children);
  const label = parts.filter((child) => typeof child === 'string').join('');

  if (process.env.NODE_ENV !== 'production' && parts.length > 0 && !label) {
    console.warn(
      'Stack.Toolbar.MenuAction on Android only supports string children. React elements will be ignored. Pass a plain string as children instead.'
    );
  }
...
export const NativeToolbarMenuAction: React.FC<NativeToolbarMenuActionProps> = (props) => {
  ...
  const label = extractLabel(props.children);
